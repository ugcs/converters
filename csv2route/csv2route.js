...
function addWaypoint( xmlRoute, lattitude, longtitude, altitude, index, waypointTemplate)
{
	var PI = 3.14159265358979;

	// create new waypoint xml structure using waypoint template
	var newWaypoint = waypointTemplate.cloneNode( true);
	...
	// We used template to create waypoint so we need to update coordinates and altitude only 
	...
	// append new waypoint 
	waypointTemplate.parentNode.appendChild( newWaypoint);
	
		
	// trick - UgCS XML parser can't load XML with more then one element per line
	// so will add text node with new line
	waypointTemplate.parentNode.appendChild( xmlRoute.createTextNode( "\r\n"));
